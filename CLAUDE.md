...
5. **Possibile duplicazione cross-fonte per le tappe RKC ASI**: alcune tappe condividono la pista con eventi già scrapeati da altre fonti (es. Misanino KCE, già noto a KRM) ma con `source_url` diverso — la dedup upsert è su `(source_url, event_date)`, quindi non rileva duplicati tra fonti diverse per lo stesso giorno/pista. Da verificare a mano se succede nella pratica.
